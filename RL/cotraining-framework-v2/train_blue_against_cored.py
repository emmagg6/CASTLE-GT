...
def train(env, input_dims, action_space,
    ...
    agent = BluePPOAgent(input_dims, action_space,
    ...
    for i_episode in range(1, max_episodes + 1):
        ...
        for t in range(max_timesteps):
            ...
            if agent.time_step % update_timestep == 0:
                ...
                ckpt_rew = os.path.join(ckpt_out, f'Blue_rewards.pth')
...
if __name__ == '__main__':
    ...
    # output folder
    ckpt_folder = '/net/data/idsgnn/coblue_against_cored_v2/'
    #ckpt_folder = os.path.join(os.getcwd(), "Models", folder)
    if not os.path.exists(ckpt_folder):
        os.makedirs(ckpt_folder)

    '''
    # coblue against fixed red
    CYBORG = CybORG(PATH, 'sim', agents={
        'Red': B_lineAgent
    })
    '''

    # coblue against internal cored ppo
    # define internal ppo red agent
    red_start_actions = []
    red_action_space=list(range(888))
    red_input_dims = 40 # observation space size for red
    red_ckpt = '/net/data/idsgnn/coRed_against_BlueReactRemove/13000.pth'
    ...
    CYBORG = CybORG(
            ...
            agents={
                "Red": ft.partial(
                    ...
                    K_epochs = K_epochs,
                    eps_clip = eps_clip,
                    start_actions = red_start_actions,
                    ckpt = red_ckpt,
                    restore = True,
    ...
    # define the external Blue ppo agent
    ...
    input_dims = 52 # for blue

    ckpt_out = ckpt_folder
    ckpt_in = None # 

    # also want to try with a blue checkpoint!!
