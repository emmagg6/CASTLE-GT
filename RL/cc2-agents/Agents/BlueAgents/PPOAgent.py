...
class PPOAgent(BaseAgent):
    ...
    def set_initial_values(self, action_space, observation=None):
        ...
        if self.restore:
            ...
            # Create a new dict to hold the updated state dict
            new_state_dict = dict()

            # Iterate over the existing state dict and update the keys
            for k, v in pretrained_model.items():
                # Check if the model was originally using DataParallel, which is indicated by the '.module.' infix
                items = k.split('.')
                if ('module' not in items):
                    items.insert(1, 'module')
                new_key = '.'.join(items)
                new_state_dict[new_key] = v  # Update the new state dict with the corrected key and original value

            # Try loading the updated state dict again
