# Cage Challenge 2 CybORG HeuristicRed adapted to be a BaseAgent
# Original HeuristicRed agent would error after exploiting the enterprise server. 
# The new GreedyAgent will continue to exploit the final exploited ip after all servers in network are exploited.
import random
...
class GreedyAgent(BaseAgent):
    ...
    def get_action(self, observation, action_space):
        ...
        if success == False:
            # Needs to be failure first because unknown (initial observation) counts as true
            self._process_last_action_failure()
        else:
            self._process_last_action_success() if self.last_action else None
            self._process_new_ips(observation)
    ...
    def _process_last_action_failure(self):
        ...
        if name in ('PrivilegeEscalate','Impact'):
            ...
            # self.ip_status[action.ip_address] = 3 # Why does killchain advance to the end if it fails to exploit???
            pass
        else:
    ...
    def _advance_killchain(self):
        ...
        elif all(list(self.ip_status.values())) > 2:
            print('exploit final server')
        else:
            ip = self._choose_ip()
    ...
    def _choose_ip(self):
        ...
        # Ok, issue is when it exploits the enterprise server it errors instead of continuing to exploit the enterprise server.
