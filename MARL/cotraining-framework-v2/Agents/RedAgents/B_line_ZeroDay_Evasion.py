...
from CybORG.Shared.Actions.ConcreteActions.ZeroDay import ZeroDay
...
class B_line_ZeroDay_Evasion_Agent(BaseAgent):
    def __init__(self):
        self.action = 0
    ...
    def get_action(self, observation, action_space):
        ...
        while True:
            ...
            # Exploit User1
                 ...
                 # If ZeroDay process is running on this machine, choose to exploit that process
                 action = ExploitRemoteService(session=session, agent='Red', ip_address=self.last_ip_address, priority=ZeroDay)
            ...
            # Exploit- Enterprise Host
                ...
                # If ZeroDay process is running on this machine, choose to exploit that process
                action = ExploitRemoteService(session=session, agent='Red', ip_address=self.target_ip_address, priority=ZeroDay)
            ...
            # Exploit- Op_Server0
                ...
                if len(info) > 0:
                    # If ZeroDay process is running on this machine, choose to exploit that process
                    action = ExploitRemoteService(agent='Red', session=session, ip_address=info[0]['Interface'][0]['IP Address'], priority=ZeroDay)
                else:
                    self.action = 0
                    continue
