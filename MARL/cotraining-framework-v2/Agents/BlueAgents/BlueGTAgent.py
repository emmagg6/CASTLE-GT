...
class BlueGTAgent(BaseAgent):
    def __init__(self, input_dims=52, action_space=[i for i in range(158)], lr=0.002, betas=[0.9, 0.990], gamma=0.99, K_epochs=4, eps_clip=0.2, restore=False, start_actions=[], ckpt=None,
        ...
        self.as_count = {}                      # dict for state to a visit count np.array -- array of visit counts for each action
        #---------------------
...
#    def train(self):
    def train(self, results=None):  # the cyborg format requires the results parameter
        ...
        #-----------------------
        cum_loss = []
        log_loss = [] # list of [old_states, old_actions, loss] lists
        log_eq = []
        #-----------------------
        ...
        for _ in range(self.K_epochs):
            ...
            #-----------------------
            eqilibrium = self.update_eq(state_values, action_values, self.possible_actions, loss)
            cum_loss.append(loss)
            log_loss.append([state_values, action_values, loss])
            log_eq.append([state_values, action_values, eqilibrium[state_values][action_values]])
            #-----------------------
...
#---------------------------------------------------------
    def update_eq(self, state, action, valid_actions, loss, eta=0.1, gamma=0.01): 
        '''
        ...
        if state not in self.eq_approx:
            self.eq_approx[state] = np.full(len(valid_actions), 1.0 / len(valid_actions))
            self.as_count[state] = np.zeros(len(valid_actions))  

        # estimate loss
        estimated_loss = loss / self.eq_approx[state][action] + 0.01

        # update eq and count
        self.eq_approx[state][action] *= np.exp(-eta * estimated_loss)
        self.eq_approx[state] /= np.sum(self.eq_approx[state])

        # return self.eq_approx[state] # will need in eval or to track
