...
class ObservationGraph:
    ...
    def _remap_sn_mask(self, k, remap):
        ...
        return (
            torch.tensor([remap[s] for s in srv]).to(self.device),
            torch.tensor([remap[u] for u in usr]).to(self.device),
            torch.tensor([
                [remap[e] for e in edge[0]],
                [remap[e] for e in edge[1]]
            ]).to(self.device),
            torch.tensor([remap[r] for r in rtrs]).to(self.device)
        )
    ...
    def get_state(self, subnets):
        ...
        transductive_routers = torch.zeros(nids.size(0), 9).to(self.device)

        x = torch.zeros(nids.size(0), self.DIM-9).to(self.device)
        for i,node in enumerate(nodes):
            ...
            if node.dim:
                ...
                x[i][offset : offset + node.dim] = torch.from_numpy(node.get_features()).to(self.device)

        x = torch.cat([x, transductive_routers], dim=1).to(self.device)
