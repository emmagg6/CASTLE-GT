...
from wrapper.globals import MAX_SERVERS, MAX_USERS
MAX_EDGES = 8

device = torch.device('cpu')#'cuda' if torch.cuda.is_available() else 'cpu')

def pad_sequence(seq, lens, padding):
    padded = torch.zeros(lens.size(0), padding, seq.size(-1)).to(device)
    mask = torch.ones(padded.size(0), padded.size(1)).to(device)
...
def extract_hosts(x, servers, n_servers, users, n_users):
    ...
    hosts = torch.cat([srv,usr], dim=1).to(device)
    mask = torch.cat([s_mask, u_mask], dim=1).to(device)
    return hosts, mask
...
class SimpleSelfAttention(nn.Module):
    ...
    def forward(self, v, mask, g=None):
        ...
        '''
        if g is None:
            g = torch.zeros((v.size(0), self.g_dim)).to(device)

        att = self.att(v)                   # B x N x h
        feat = self.feat(v)                 # B x N x h
        out = (att*feat*mask).sum(dim=1)    # B x h

        g_ = self.glb(torch.cat([out,g], dim=-1).to(device))  # B x g
...
class InductiveActorNetwork(nn.Module):
    ...
    def forward(self, x, ei, global_vec, servers, n_servers, users, n_users, action_edges, multi_subnet):
        ...
        rtr_mask = torch.ones(rtrs.size(0), 9, 1).to(device)
        ...
        rtr = x[rtrs]
        v = torch.cat([v, rtr], dim=1).to(device)
        mask = torch.cat([mask, rtr_mask], dim=1).to(device)
        g = self.g0_attn(v,mask, g=g0)

        # Layer 1
        x = torch.relu(self.conv1(x, ei)).to(device)
        v,mask = extract_hosts(x, servers, n_servers, users, n_users)
        rtr = x[rtrs]
        v = torch.cat([v, rtr], dim=1).to(device)
        mask = torch.cat([mask, rtr_mask], dim=1).to(device)
        ...
        # Layer 2
        x = torch.relu(self.conv2(x, ei)).to(device)
        v,mask = extract_hosts(x, servers, n_servers, users, n_users)
        rtr = x[rtrs]
        v = torch.cat([v, rtr], dim=1).to(device)
        mask = torch.cat([mask, rtr_mask], dim=1).to(device)
        ...
        if self.concat_edges:
            edge_a = self.edge_actions(torch.cat([src,dst], dim=-1).to(device))
        else:
        ...
        out = torch.cat([node_a, edge_a, glb_a], dim=-1).to(device)
...
class InductiveGraphPPOAgent():
    ...
    def __init__(self, in_dim, gamma=0.99, lmbda=0.95, clip=0.1, bs=5, epochs=6,
                 a_kwargs=dict(), c_kwargs=dict(), training=True, concat_edges=False, device=torch.device('cpu')):
        self.device = device

        self.actor = InductiveActorNetwork(in_dim, concat_edges=concat_edges, **a_kwargs).to(device)
        self.critic = InductiveCriticNetwork(in_dim, **c_kwargs).to(device)
    ...
    def learn(self, verbose=False):
        ...
        for e in range(self.epochs):
            ...
            # Calculate advantage 
            advantages = r - torch.tensor(v).to(self.device)
            ...
            for b_idx,b in enumerate(batches):
                ...
                new_probs = dist.log_prob(torch.tensor(a_).to(self.device))
                old_probs = torch.tensor([p[i] for i in b]).to(self.device)
                ...
                actor_loss = torch.min(r_theta*a_t, clipped_r_theta*a_t).to(self.device)
